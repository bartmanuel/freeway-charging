...
// Supabase (NDW bulk data) is the primary source; OCM is the fallback for
// sparse/uncharted areas. The apiKey parameter is kept for compatibility
// but is no longer used — the Worker holds its own key server-side.
export async function fetchStationsAlongRoute(
  ...
  const res = await fetch(`${WORKER_URL}/api/stations/corridor`, {
    ...
    body: JSON.stringify({ ...bbox, minPowerKw: 50, encodedPolyline }),
  });
